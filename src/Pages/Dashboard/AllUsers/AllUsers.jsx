...
const AllUsers = () => {
    ...
    //     queryKey: ['users'],
    //     queryFn: async () => {
    //         const res = await fetch("https://summer-school-server-tau.vercel.app/users")
    //         return res.json();
    //     }
    // })
    ...
    const handleMakeAdmin = user => {
        fetch(`https://summer-school-server-tau.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleMakeInstructor = user => {
        fetch(`https://summer-school-server-tau.vercel.app/users/instructor/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleDeleteUser = (user) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://summer-school-server-tau.vercel.app/users/${user._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
